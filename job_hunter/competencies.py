...
reading each project's code — the ``source_ref`` field exists so every
"hands-on" claim can point at the file/function that proves it, not at Kai's
own recollection. This module only defines the SHAPE and the sync logic; it
does not itself decide what belongs in any bucket.
...
class CompetencyEntry:
    """One skill/term. ``resume_line`` is only meaningful for bucket="core".

    ``source_ref`` is the SOURCE OF TRUTH pointer (project + file/function),
    e.g. "job_hunter/scoring.py:topic_prefilter" — the claim is only as good
    as this pointer; an entry with no source_ref is content still awaiting
    the manual code-reading pass, not something to take on faith.
    """
...
def parse_competencies(
    ...
    Expected shape: {bucket: [ {term_ru, term_en, ...}, ... ], ...} keyed by
    the BUCKETS names. Unknown top-level keys and rows missing BOTH term_ru
    and term_en are skipped — best-effort, content review happens by reading
    the file, not by failing the whole load over one bad row.
    """
...
def sync_with_market(
    ...
    display names stack_analytics produces (e.g. "PyTorch", "LLM"). This is a
    deliberately coarse first pass — an entry phrased differently than the
    market's exact vocabulary (e.g. "prompt engineering" vs a market string
    "Prompt Engineer") will show 0 until the content-filling pass tunes
    term_en to match, or stack_analytics._CANONICAL grows an alias. Getting
    that right needs REAL entries to test against, which is explicitly out of
    scope for this pass (see module docstring).

    Returns:
      matched — one row per input entry (dict of its fields + market_count/
        market_pct), in input order.
